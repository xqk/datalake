...
import DeMainContainer from '@/components/datalake/DeMainContainer'
import DeContainer from '@/components/datalake/DeContainer'
import DeAsideContainer from '@/components/datalake/DeAsideContainer'
...
export default {
  ...
  methods: {
    ...
    exportExcelDownload(snapshot, width, height) {
      ...
      const excelHeaderKeys = JSON.parse(JSON.stringify(this.chart.data.fields)).map(item => item.datalakeName)
