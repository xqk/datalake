...
public class DataSetTableFieldsService {
    ...
    public DatasetTableField save(DatasetTableField datasetTableField) {
        if (StringUtils.isEmpty(datasetTableField.getId())) {
            ...
            // 若datalakename为空，则用MD5(id)作为datalakename
            if (StringUtils.isEmpty(datasetTableField.getDatalakeName())) {
                datasetTableField.setDatalakeName(TableUtils.columnName(datasetTableField.getId()));
            }
