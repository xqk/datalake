...
import io.datalake.commons.constants.*;
import io.datalake.commons.exception.DEException;
import io.datalake.commons.utils.*;
...
import io.datalake.controller.request.dataset.DataSetTaskRequest;
import io.datalake.controller.response.DataSetDetail;
import io.datalake.dto.SysLogDTO;
import io.datalake.dto.dataset.*;
import io.datalake.dto.dataset.union.UnionDTO;
import io.datalake.dto.dataset.union.UnionItemDTO;
import io.datalake.dto.dataset.union.UnionParamDTO;
import io.datalake.exception.DataLakeException;
import io.datalake.ext.ExtDataSetGroupMapper;
import io.datalake.ext.ExtDataSetTableMapper;
import io.datalake.ext.UtilMapper;
...
import io.datalake.plugins.common.base.mapper.*;
...
import io.datalake.provider.DDLProvider;
import io.datalake.provider.ProviderFactory;
import io.datalake.provider.datasource.JdbcProvider;
import io.datalake.service.engine.EngineService;
import io.datalake.service.sys.SysAuthService;
...
import static io.datalake.commons.constants.ColumnPermissionConstants.Desensitization_desc;
...
public class DataSetTableService {
                ...
                for (String s : map.keySet()) {
                    ...
                    excelSheetDataList.forEach(excelSheetData -> {
                        ...
                        if (checkIsRepeat(fieldArray)) {
                            DataLakeException.throwException(Translator.get("i18n_excel_field_repeat"));
                        }
        ...
        for (ExcelSheetData sheet : datasetTable.getSheets()) {
            ...
            if (datasetTable.getEditType() == 0) {
                ...
                if (!oldFields.equals(newFields)) {
                    DataLakeException.throwException(Translator.get("i18n_excel_column_inconsistent"));
                }
                oldFields = newFields;
            }
    ...
    public DatasetTable save(DataSetTableRequest datasetTable) throws Exception {
        ...
        if (StringUtils.isEmpty(datasetTable.getId())) {
            ...
            // 添加表成功后，获取当前表字段和类型，抽象到datalake数据库
            if (insert == 1) {
    ...
    public Map<String, Object> getPreviewData(DataSetTableRequest dataSetTableRequest, Integer page, Integer pageSize,
        ...
        String[] fieldArray = fields.stream().map(DatasetTableField::getDatalakeName).toArray(String[]::new);
    ...
    public Map<String, Object> getSQLPreview(DataSetTableRequest dataSetTableRequest) throws Exception {
        ...
        if (StringUtils.isEmpty(sql)) {
            DataLakeException.throwException(Translator.get("i18n_sql_not_empty"));
        }
        QueryProvider qp = ProviderFactory.getQueryProvider(ds.getType());
                ...
                for (TableField tableField : fields) {
                    if (StringUtils.equalsIgnoreCase(tableField.getFieldName(),
                            TableUtils.fieldName(
                                    datasetTableField.getTableId() + "_" + datasetTableField.getDatalakeName()))
    ...
    private String getCustomViewSQL(DataTableInfoDTO dataTableInfoDTO, List<DataSetTableUnionDTO> list) {
        ...
        dataTableInfoDTO.getList().forEach(ele -> {
            ...
            String[] array = fields.stream()
                    .map(f -> table + "." + f.getDatalakeName() + " AS "
                            + TableUtils.fieldName(ele.getTableId() + "_" + f.getDatalakeName()))
                    .toArray(String[]::new);
                ...
                for (DataSetTableUnionDTO dto : list) {
                    ...
                    if (StringUtils.equals(dto.getTargetTableId(), dataTableInfoCustomUnion.getTableId())) {
                        ...
                        join.append(convertUnionTypeToSQL(dto.getSourceUnionRelation()))
                                ...
                                .append(sourceField.getDatalakeName())
                                .append(" = ")
                                .append(TableUtils.tableName(dto.getTargetTableId())).append(".")
                                .append(targetField.getDatalakeName());
                    }
                }
            }
    ...
    private Map<String, Object> getUnionSQLDoris(DataTableInfoDTO dataTableInfoDTO) {
        ...
        for (UnionDTO unionDTO : union) {
            ...
            String[] array = fields.stream()
                    .map(f -> table + "." + f.getDatalakeName() + " AS "
                            + TableUtils.fieldName(tableId + "_" + f.getDatalakeName()))
                    .toArray(String[]::new);
            ...
            for (UnionParamDTO unionParamDTO : unionList) {
                ...
                for (int i = 0; i < unionParamDTO.getUnionFields().size(); i++) {
                    ...
                    join.append(TableUtils.tableName(parentTable.getId())).append(".")
                            .append(parentField.getDatalakeName())
                            .append(" = ")
                            .append(TableUtils.tableName(currentTable.getId())).append(".")
                            .append(currentField.getDatalakeName());
    ...
    public List<DatasetTableField> saveExcelTableField(String datasetTableId, List<TableField> fields, boolean insert) {
        ...
        if (CollectionUtils.isNotEmpty(fields)) {
            for (int i = 0; i < fields.size(); i++) {
                ...
                datasetTableField.setDatalakeName(TableUtils.columnName(filed.getFieldName()));
        ...
        if (StringUtils.equalsIgnoreCase(datasetTable.getType(), "db") || StringUtils.equalsIgnoreCase(datasetTable.getType(), "api")) {
            ...
            if (datasetTable.getMode() == 1) {
                ...
                list.forEach(ele -> {
                    ...
                    listByIds.forEach(f -> f.setDatalakeName(
                            TableUtils.fieldName(ele.getTableId() + "_" + f.getDatalakeName())));
            ...
            if (datasetTable.getMode() == 1) {
                ...
                for (DatasetTableField field : fieldList) {
                    for (TableField tableField : fields) {
                        if (StringUtils.equalsIgnoreCase(
                                TableUtils.fieldName(field.getTableId() + "_" + field.getDatalakeName()),
                                tableField.getFieldName())) {
                            tableField.setRemarks(field.getName());
            ...
            for (int i = 0; i < fields.size(); i++) {
                ...
                if (CollectionUtils.isNotEmpty(datasetTableFields)) {
                    ...
                    if (datasetTable.getMode() == 1 && StringUtils.equalsIgnoreCase("union", datasetTable.getType())) {
                        datasetTableField.setDatalakeName(filed.getFieldName());
                    } else {
                        datasetTableField.setDatalakeName(TableUtils.columnName(filed.getFieldName()));
                    }
                    datasetTableField.setType(filed.getFieldType());
    ...
    private void checkColumes(DatasetTableIncrementalConfig datasetTableIncrementalConfig) throws Exception {
        ...
        if (StringUtils.isNotEmpty(datasetTableIncrementalConfig.getIncrementalAdd())
            ...
            try {
                ...
                DataLakeException.throwException(Translator.get("i18n_check_sql_error") + e.getMessage());
            }

            if (!originNameFileds.equals(sqlFileds)) {
                DataLakeException.throwException(Translator.get("i18n_sql_add_not_matching") + sqlFileds.toString());
            }
        }
        if (StringUtils.isNotEmpty(datasetTableIncrementalConfig.getIncrementalDelete())
            ...
            if (!originNameFileds.equals(sqlFileds)) {
                DataLakeException.throwException(Translator.get("i18n_sql_delete_not_matching") + sqlFileds.toString());
            }
        }
    }
        ...
        if (StringUtils.isNotEmpty(tableId)) {
            ...
            if(editType == 1){
                ...
                if (retrunSheetDataList.size() == 0) {
                    DataLakeException.throwException(Translator.get("i18n_excel_column_change"));
                }
            }else {
