...
import io.datalake.commons.model.AuthURD;
...
import io.datalake.dto.dataset.ExcelSheetData;
import io.datalake.dto.datasource.*;
import io.datalake.exception.DataLakeException;
import io.datalake.ext.ExtChartViewMapper;
...
import io.datalake.plugins.common.constants.DeTypeConstants;
...
import io.datalake.service.message.DeMsgutil;
...
public class ExtractDataService {
    ...
    private static final String root_path = "/opt/datalake/data/kettle/";
            ...
            case all_scope:  // 全量更新
                try {
                    ...
                    if (!engineService.isSimpleMode()) {
                        ...
                        generateJobFile("all_scope", datasetTable, datasetTableFields.stream().map(DatasetTableField::getDatalakeName).collect(Collectors.joining(",")));
                    ...
                    if (ops.equalsIgnoreCase("替换")) {
                        ...
                        for (DatasetTableField oldFiled : oldFileds) {
                            ...
                            for (DatasetTableField datasetTableField : datasetTableFields) {
                                if (oldFiled.getDatalakeName().equalsIgnoreCase(datasetTableField.getDatalakeName()) && oldFiled.getDeExtractType().equals(datasetTableField.getDeExtractType())) {
                                    delete = false;
                                }
                            }
                            if (delete) {
            ...
            case add_scope: // 增量更新
                try {
                    ...
                    if (!engineService.isSimpleMode()) {
                        ...
                        generateJobFile("incremental_add", datasetTable, datasetTableFields.stream().map(DatasetTableField::getDatalakeName).collect(Collectors.joining(",")));
    ...
    private void extractApiData(DatasetTable datasetTable, Datasource datasource, List<DatasetTableField> datasetTableFields, String extractType) throws Exception {
        ...
        String columns = datasetTableFields.stream().map(DatasetTableField::getDatalakeName).collect(Collectors.joining(",")) + ",datalake_uuid";
    ...
    private void extractDataByKettle(DatasetTable datasetTable, Datasource datasource, List<DatasetTableField> datasetTableFields, String extractType, String selectSQL) throws Exception {
        ...
        generateJobFile(extractType, datasetTable, datasetTableFields.stream().map(DatasetTableField::getDatalakeName).collect(Collectors.joining(",")));
    ...
    private void extractData(DatasetTable datasetTable, String extractType) throws Exception {
        ...
        if (!transStatus.getStatusDescription().equals("Finished")) {
            DataLakeException.throwException(transStatus.getLoggingString());
            return;
        }
        executing = true;
        ...
        if (jobStatus.getStatusDescription().equals("Finished")) {
            ...
            DataLakeException.throwException(jobStatus.getLoggingString());
        }
    }
    ...
    private void generateJobFile(String extractType, DatasetTable datasetTable, String columnFields) throws Exception {
        ...
        String columns = columnFields + ",datalake_uuid";
        String streamLoadScript = "";
        if (kettleFilesKeep) {
            streamLoadScript = shellScript;
        } else {
    ...
    private StepMeta excelInputStep(String Info, List<DatasetTableField> datasetTableFields) {
        ...
        for (int i = 0; i < datasetTableFields.size(); i++) {
            ...
            field.setName(datasetTableFields.get(i).getDatalakeName());
    ...
    private StepMeta outputStep(String dorisOutputTable, List<DatasetTableField> datasetTableFields, Datasource datasource) {
        ...
        if (datasource.getType().equalsIgnoreCase(DatasourceTypes.oracle.name())) {
            ...
            TextFileField textFileField = new TextFileField();
            textFileField.setName("datalake_uuid");
            textFileField.setType("String");
            ...
            for (int i = 0; i < datasetTableFields.size(); i++) {
                TextFileField textFileField = new TextFileField();
                textFileField.setName(datasetTableFields.get(i).getDatalakeName());
    ...
    private StepMeta udjc(List<DatasetTableField> datasetTableFields, DatasourceTypes datasourceType) {
        ...
        for (DatasetTableField datasetTableField : datasetTableFields) {
            if (datasetTableField.getDeExtractType().equals(DeTypeConstants.DE_BINARY)) {
                handleBinaryTypeCode.append("\n").append(handleBinaryType.replace("FIELD", datasetTableField.getDatalakeName()));
            }
        }

        UserDefinedJavaClassMeta userDefinedJavaClassMeta = new UserDefinedJavaClassMeta();
        List<UserDefinedJavaClassMeta.FieldInfo> fields = new ArrayList<>();
        UserDefinedJavaClassMeta.FieldInfo fieldInfo = new UserDefinedJavaClassMeta.FieldInfo("datalake_uuid", ValueMetaInterface.TYPE_STRING, -1, -1);
        ...
        if (datasourceType.equals(DatasourceTypes.oracle) || datasourceType.equals(DatasourceTypes.db2)) {
            ...
            Column_Fields = datasetTableFields.stream().map(DatasetTableField::getDatalakeName).collect(Collectors.joining(","));
        }
    ...
    private final static String code = "import org.pentaho.di.core.row.ValueMetaInterface;\n" +
            ...
            "\n" +
            "  String md5 = md5(str);\n" +
            "  get(Fields.Out, \"datalake_uuid\").setValue(r, md5);\n" +
            "\n" +
