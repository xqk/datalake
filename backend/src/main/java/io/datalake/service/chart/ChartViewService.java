...
import io.datalake.commons.constants.JdbcConstants;
import io.datalake.commons.exception.DEException;
import io.datalake.commons.model.PluginViewSetImpl;
...
import io.datalake.controller.request.chart.*;
import io.datalake.controller.response.ChartDetail;
import io.datalake.controller.response.DataSetDetail;
import io.datalake.dto.chart.*;
...
import io.datalake.plugins.common.base.domain.*;
import io.datalake.plugins.common.base.mapper.ChartViewCacheMapper;
import io.datalake.plugins.common.base.mapper.ChartViewMapper;
import io.datalake.plugins.common.base.mapper.PanelViewMapper;
import io.datalake.plugins.common.dto.chart.ChartFieldCompareDTO;
...
import io.datalake.service.chart.util.ChartDataBuild;
import io.datalake.service.dataset.DataSetTableFieldsService;
import io.datalake.service.dataset.DataSetTableService;
import io.datalake.service.dataset.DataSetTableUnionService;
import io.datalake.service.dataset.PermissionService;
import io.datalake.service.datasource.DatasourceService;
import io.datalake.service.engine.EngineService;
import io.datalake.service.panel.PanelGroupExtendDataService;
...
public class ChartViewService {
    ...
    public ChartViewDTO getOne(String id, String queryFrom) {
        try {
            ...
            if (result == null) {
                DataLakeException.throwException(Translator.get("i18n_chart_delete"));
            }
            return result;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<String[]> sqlData(ChartViewDTO view, ChartExtRequest requestList, boolean cache, String fieldId) throws Exception {
        ...
        //将没有权限的列删掉
        List<String> datalakeNames = columnPermissionFields.stream().map(DatasetTableField::getDatalakeName).collect(Collectors.toList());
        datalakeNames.add("*");
        fieldCustomFilter = fieldCustomFilter.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName()) && datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
        extStack = extStack.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName()) && datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
        extBubble = extBubble.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName()) && datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
        drill = drill.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName()) && datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
        ...
        switch (view.getType()) {
            case "label":
                xAxis = xAxis.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName()) && datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
                yAxis = new ArrayList<>();
            ...
            case "liquid":
                xAxis = new ArrayList<>();
                yAxis = yAxis.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName()) && datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
            ...
            case "table-info":
                yAxis = new ArrayList<>();
                xAxis = xAxis.stream().filter(item -> datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
                if (CollectionUtils.isEmpty(xAxis)) {
                    return new ArrayList<String[]>();
                }
                break;
            case "table-normal":
                xAxis = xAxis.stream().filter(item -> datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
                yAxis = yAxis.stream().filter(item -> datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
                break;
            default:
                xAxis = xAxis.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName()) && datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
                yAxis = yAxis.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName()) && datalakeNames.contains(item.getDatalakeName())).collect(Collectors.toList());
                ...
                if (StringUtils.isNotEmpty(fieldId)) {
                    ...
                    if (request.getIsTree()) {
                        ...
                        for (String fId : fieldIds) {
                            ...
                            if (!desensitizationList.contains(datasetTableField.getDatalakeName()) && datalakeNames.contains(datasetTableField.getDatalakeName())) {
