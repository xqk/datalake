...
import io.datalake.controller.response.ExistLdapUser;
...
import io.datalake.service.sys.SysRoleService;
...
public class SysUserController {
    ...
    public void updatePersonInfo(@RequestBody SysUserCreateRequest request) {
        ...
        if (!request.getUserId().equals(userId) || request.getEmail() == null || request.getNickName() == null) {
            DataLakeException.throwException(Translator.get("i18n_wrong_content"));
        }
        // 再次验证，匹配格式
        if (StringUtils.isNotBlank(request.getPhone()) && !request.getPhone().matches("^1[3|4|5|7|8][0-9]{9}$")) {
            DataLakeException.throwException(Translator.get("i18n_wrong_tel"));
        }
        if (!request.getEmail().matches("^[a-zA-Z0-9_._-]+@[a-zA-Z0-9_-]+(\\.[a-zA-Z0-9_-]+)+$")) {
            DataLakeException.throwException(Translator.get("i18n_wrong_email"));
        }
        if (!(2 <= request.getNickName().length() && request.getNickName().length() <= 50)) {
            DataLakeException.throwException(Translator.get("i18n_wrong_name_format"));
        }
