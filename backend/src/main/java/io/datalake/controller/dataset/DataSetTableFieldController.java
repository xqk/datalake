...
import io.datalake.auth.filter.F2CLinkFilter;
...
import io.datalake.controller.request.dataset.MultFieldValuesRequest;
import io.datalake.controller.response.DatasetTableField4Type;
...
import io.datalake.service.dataset.DataSetFieldService;
...
public class DataSetTableFieldController {
    ...
    public List<DatasetTableField> listWithPermission(@PathVariable String tableId) {
        ...
        fields = fields.stream().filter(item -> !desensitizationList.contains(item.getDatalakeName())).collect(Collectors.toList());
        return fields;
    }
