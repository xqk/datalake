...
import io.datalake.auth.entity.TokenInfo.TokenInfoBuilder;
...
public class JWTUtils {
    ...
    public static TokenInfo tokenInfoByToken(String token) {
        ...
        if (StringUtils.isEmpty(username) || ObjectUtils.isEmpty(userId)) {
            DataLakeException.throwException("token格式错误！");
        }
    ...
    public static boolean loginExpire(String token) {
        if (Login_Interval == 0) {
            // 默认超时时间是8h
            Long minute = CommonBeanFactory.getBean(Environment.class).getProperty("datalake.login_timeout", Long.class,
                    8 * 60L);
            // 分钟换算成毫秒
