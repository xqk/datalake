package io.datalake.provider.datasource;
...
import io.datalake.plugins.common.base.mapper.DeDriverMapper;
...
import io.datalake.plugins.datasource.provider.DefaultJdbcProvider;
import io.datalake.plugins.datasource.provider.ExtendedJdbcClassLoader;
...
public class JdbcProvider extends DefaultJdbcProvider {
    ...
    public List<TableField> getTableFileds(DatasourceRequest datasourceRequest) throws Exception {
        ...
        try (Connection connection = getConnectionFromPool(datasourceRequest)) {
            ...
            if (datasourceRequest.getDatasource().getType().equalsIgnoreCase("ds_doris") || datasourceRequest.getDatasource().getType().equalsIgnoreCase("StarRocks")) {
                ...
                DataLakeException.throwException(Translator.get("i18n_datasource_connect_error") + e.getMessage());
            }
    ...
    public List<String[]> getData(DatasourceRequest dsr) throws Exception {
        ...
        try (Connection connection = getConnectionFromPool(dsr); Statement stat = connection.createStatement(); ResultSet rs = stat.executeQuery(dsr.getQuery())) {
            ...
            DataLakeException.throwException("SQL ERROR" + e.getMessage());
        } catch (Exception e) {
            DataLakeException.throwException("Data source connection exception: " + e.getMessage());
        }
        return list;
    }
    ...
    public String checkStatus(DatasourceRequest datasourceRequest) throws Exception {
        ...
        try (Connection con = getConnection(datasourceRequest); Statement statement = con.createStatement(); ResultSet resultSet = statement.executeQuery(queryStr)) {
            ...
            io.datalake.plugins.common.exception.DataLakeException.throwException(e.getMessage());
        }
        return "Success";
    }
        ...
        switch (datasourceType) {
            ...
            case hive:
                ...
                if (StringUtils.isNotEmpty(hiveConfiguration.getAuthMethod()) && hiveConfiguration.getAuthMethod().equalsIgnoreCase("kerberos")) {
                    System.setProperty("java.security.krb5.conf", "/opt/datalake/conf/krb5.conf");
                    ...
                    loginUserFromKeytab.invoke(null, hiveConfiguration.getUsername(), "/opt/datalake/conf/" + hiveConfiguration.getPassword());
                } else {
