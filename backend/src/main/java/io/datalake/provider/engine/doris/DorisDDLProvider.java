...
import io.datalake.commons.utils.TableUtils;
import io.datalake.dto.datasource.DorisConfiguration;
import io.datalake.plugins.common.base.domain.DatasetTableField;
import io.datalake.plugins.common.base.domain.Datasource;
import io.datalake.provider.DDLProviderImpl;
...
public class DorisDDLProvider extends DDLProviderImpl {
    private static final String creatTableSql = "CREATE TABLE IF NOT EXISTS `TABLE_NAME`" +
            "Column_Fields" +
            "UNIQUE KEY(datalake_uuid)\n" +
            "DISTRIBUTED BY HASH(datalake_uuid) BUCKETS BUCKETS_NUM\n" +
    ...
    private String createDorisTableColumnSql(final List<DatasetTableField> datasetTableFields) {
        StringBuilder Column_Fields = new StringBuilder("datalake_uuid  varchar(50), `");
        for (DatasetTableField datasetTableField : datasetTableFields) {
            Column_Fields.append(datasetTableField.getDatalakeName()).append("` ");
