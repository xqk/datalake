package io.datalake.provider.query.mysql;
...
import io.datalake.plugins.common.constants.MySQLConstants;
    ...
    public String createRawQuerySQL(String table, List<DatasetTableField> fields, Datasource ds) {
        String[] array = fields.stream().map(f -> {
            ...
            if (f.getDeExtractType() == 4) { // 处理 tinyint
                stringBuilder.append("concat(`").append(f.getOriginName()).append("`,'') AS ").append(f.getDatalakeName());
            } else {
                stringBuilder.append("`").append(f.getOriginName()).append("` AS ").append(f.getDatalakeName());
            }
